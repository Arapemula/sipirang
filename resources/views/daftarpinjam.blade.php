            ...
            <div class="col-md-12 p-0">
                <div class="card-body">
                    @if (session()->has('rentSuccess'))
                        <div class="alert alert-success text-center alert-dismissible fade show mx-auto" style="margin-top: 50px;" role="alert">
                            {{ session('rentSuccess') }}
                    ...
                    <div class="d-flex">
                        <div class="table-responsive" style="width: 100%; margin-right: -75px;">
                            <table class="table align-middle table-hover table-dark" style="width: 100%; color: #f8f9fa; table-layout: auto;">
                                ...
                                <tbody>
                                    @forelse ($userRents as $index => $rent)
                                        ...
                                        ">
                                            ...
                                            <td>
                                                <a href="/showruang/{{ $rent->room->code }}" class="text-decoration-none" style="color: #000000;">{{ $rent->room->code }}</a>
                                            </td>
                        ...
                        <div class="d-none d-md-flex flex-column align-items-end justify-content-center gap-2" style="width: 15%;">
                            @if ($currentPage > 1)
                                <a href="{{ $userRents->url($currentPage - 1) }}" class="btn btn-outline-secondary p-2 fs-5" style="width: 50px;">↑</a>
                            @else
                            ...
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $userRents->url($i) }}" class="btn btn-outline-secondary p-2 fs-5" style="width: 50px;">{{ $i }}</a>
                                @endif
                            @endfor
                    ...
                    <div class="d-flex d-md-none justify-content-end mt-3 w-100">
                        <div class="btn-group" role="group" aria-label="Pagination">
                            @if ($currentPage > 1)
                                <a href="{{ $userRents->url($currentPage - 1) }}" class="btn btn-outline-secondary">←</a>
                            @else
                            ...
                            @for ($i = $start; $i <= $end; $i++)
                                ...
                                @else
                                    <a href="{{ $userRents->url($i) }}" class="btn btn-outline-secondary">{{ $i }}</a>
                                @endif
                            @endfor
    ...
    <script>
        ...
        var calendar = new FullCalendar.Calendar(calendarEl, {
            ...
            @foreach($calendarEvents as $rent)
            {
                ...
                color: 
                @if($rent->status == 'dipinjam') '#dc3545' 
                @elseif($rent->status == 'selesai') '#28a745' 
                @else '#ffc107' 
                @endif,
                extendedProps: {
